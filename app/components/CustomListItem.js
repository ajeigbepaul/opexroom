...
const CustomListItem = ({ id, Title, index, goToChat }) => {
  ...
  return (
    ...
    >
      <View className="flex-row space-x-3 bg-gray-100 p-4 border-b border-gray-50 my-2 mx-2 rounded-lg shadow-lg">
        {URL[0] ? (
        ...
        <View className="w-8 h-6 rounded-md bg-green-200 items-center justify-center ">
          <Text className="font-semibold">{chatCount.length}</Text>
        </View>
      </View>
    </TouchableWithoutFeedback>
  );
